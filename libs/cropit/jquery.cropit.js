	  ...
	  _createClass(Cropit, [{
	    ...
	    value: function onFileChange(e) {
	      ...
	      if (this.$fileInput.get(0).files) {
	      	limitsize = 10;
	      	image = this.$fileInput.get(0).files[0];
	      	$('.cropit-image-control').find(".error-msg").fadeOut(300);
            if((image.size / (1024*1024)) > limitsize) {
            	$('.cropit-image-control').find(".error-msg").text("Zəhmət olmasa həcmi 10 MB-dən kiçik şəkil seçin").fadeIn(300);
        	} else {
